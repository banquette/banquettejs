...
import { RemoteException } from "@banquette/model-api/module/remote/remote.exception";
import { RemoteModule } from "@banquette/model-api/module/remote/remote.module";
import { ApiTransformerSymbol } from "@banquette/model-api/transformer/root/api";
...
@Module()
@Component({
    name: 'bt-form-generic',
    factory: () => Injector.Get(GenericComponent)
})
export default class GenericComponent extends Vue {
     ...
     */
    @Expose() public vm!: GenericComponent;
    ...
    @Expose() public load(): void {
        ...
        const doLoad = async () => {
            ...
            if (this.loadRemote.isApplicable) {
                const remoteData: any = await this.loadRemotely();
                if (this.modelType !== null) {
                    this.model = remoteData;
                }
            } else if (this.modelType !== null) {
                // TODO: the type of transformer must be configurable.
                const transformResult = this.transformService.transformInverse(this.loadData || {}, this.modelType, PojoTransformerSymbol);
                if (transformResult.promise !== null) {
                    await transformResult.promise;
                }
                this.model = transformResult.result;
            }
            if (this.model) {
    ...
    @Expose() public submit(): void {
        ...
        const doSubmit = async () => {
            ...
            if (this.model !== null) {
                // TODO: the type of transformer must be configurable.
                const transformResult = this.transformService.transform(this.model, PojoTransformerSymbol);
                if (transformResult.promise !== null) {
                    await transformResult.promise;
                }
                submitValue = transformResult.result;
            }
            if (this.persistRemote.isApplicable) {
                this.form.disable();
                this.updateState(Action.Persist, Status.Working);
                const response: HttpResponse<any> = this.persistRemote.send(submitValue, {}, [FORM_GENERIC_PERSIST_REQUESTS_TAG]);
                try {
    ...
    /**
     * Fetch remote data to initialize the form with.
     */
    private loadRemotely(): Promise<any> {
        return this.loadRemote.send(null, this.loadUrlParams, [FORM_GENERIC_LOAD_REQUESTS_TAG]).promise.then(async (response: HttpResponse<any>) => {
            if (this.modelType !== null) {
                const transformResult = this.transformService.transformInverse(response, this.modelType /* TODO: try to put an invalid string here to test the failure behavior */, ApiTransformerSymbol);
                await transformResult.promise;
