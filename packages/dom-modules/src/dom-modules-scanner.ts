import { Service, InjectableMetadataInterface, Injector, MetadataContainer } from '@banquette/dependency-injection';
...
import { isNullOrUndefined, isUndefined, Constructor, Pojo } from '@banquette/utils-type';
...
export class DomModulesScannerService {
    ...
    private modulesAttributeSelectors: {
        [key: string]: DomModuleConstructor;
    } | null = null;

    /**
     * Map DOM elements to their associated DomModule instances.
     */
    private existingModules: WeakMap<
        HTMLElement,
        Record<string, DomModuleInterface>
    > = new WeakMap<HTMLElement, Record<string, DomModuleInterface>>();
    ...
    public scan(): void {
        ...
        if (this.modulesAttributeSelectors === null) {
            this.modulesAttributeSelectors =
                this.computeModulesAttributeSelectors();
        }
        for (const selector of Object.keys(this.modulesAttributeSelectors)) {
            document.querySelectorAll(`[${selector}]`).forEach(
                (function (attrName: string, moduleCtor: DomModuleConstructor) {
                    return function (element: Element) {
                        ...
                        const moduleInstance: DomModuleInterface = Injector.Get(
                            moduleCtor as Constructor<any>
                        );
                        let options: Pojo = {};
                        ...
                        if (!isUndefined(existingModules[attrName])) {
                            throw new UsageException(
                                `Multiple initialization of the DOM module "${moduleCtor[MODULE_NAME_CTOR_ATTR]}".`
                            );
                        }
                        if (attrValue) {
                            if (attrValue[0] === '{') {
                                ...
                                const defaultOptionName: string | null =
                                    moduleInstance.getDefaultOptionName();
                                if (defaultOptionName === null) {
                                    throw new UsageException(
                                        `No default option name has been defined for the module "${moduleCtor[MODULE_NAME_CTOR_ATTR]}".`
                                    );
                                }
    ...
    private computeModulesAttributeSelectors(): {
        ...
        const metadata: InjectableMetadataInterface[] =
            MetadataContainer.GetForTag(ModuleInjectorTag);
        const ctors: DomModuleConstructor[] = metadata.reduce(
            (
                acc: DomModuleConstructor[],
                item: InjectableMetadataInterface
            ) => {
                if (
                    item.tags.indexOf(ModuleInjectorTag) > -1 &&
                    !isUndefined((item.ctor as any)[MODULE_NAME_CTOR_ATTR])
                ) {
        ...
        for (const ctor of ctors) {
            const attrName: string =
                DomModulesScannerService.MODULES_HTML_ATTRIBUTES_PREFIX +
                kebabCase(ctor[MODULE_NAME_CTOR_ATTR]);
