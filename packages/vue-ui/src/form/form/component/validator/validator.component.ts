...
import { getComponentName } from "@banquette/vue-typescript/utils/get-component-name";
...
export abstract class ValidatorComponent extends Vue {
    ...
    public get targets(): string[] {
        if (this.parentFormComponent !== null &&
            ...
            this.parentFormComponent.vm.control &&
            this.parentFormComponent.vm.control.path
        ) {
            return [this.parentFormComponent.vm.control.path];
        }
    ...
    public mounted(): void {
        ...
        while ($parent) {
            ...
            } else if (getComponentName($parent) === 'bt-form') {
