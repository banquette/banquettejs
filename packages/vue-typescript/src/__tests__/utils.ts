...
import { VUE_CLASS_COMPONENT_OPTIONS, COMPONENT_INSTANCE } from "../constants";
...
export function getChildComponentInstance<T extends Constructor>(wrapper: VueWrapper<any>, ctor: T): InstanceType<T> {
    const opts = (ctor as any)[VUE_CLASS_COMPONENT_OPTIONS];
    if (isUndefined(opts)) {
        throw `"${ctor.name}" is not a vue-typescript component.`;
    }
    const component: any = wrapper.findComponent(opts);
    if (isObject(component) && isObject(component.vm) && isObject(component.vm.$) && isObject(component.vm.$[COMPONENT_INSTANCE])) {
        return component.vm.$[COMPONENT_INSTANCE];
    }
