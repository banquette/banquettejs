/**
 * From https://github.com/bd808/php-unserialize-js/blob/main/phpUnserialize.spec.js
 */
import { UsageException } from '@banquette/exception';
import { phpUnserialize } from '../src/php-unserialize';

describe('Primative values', () => {
    it('can parse a string', () => {
        expect(phpUnserialize('s:3:"foo";')).toBe('foo');
        expect(phpUnserialize('s:17:"bl\u00e5b\u00e6rsyltet\u00f8y";')).toBe(
            'blåbærsyltetøy'
        );
        expect(phpUnserialize('s:17:"blåbærsyltetøy";')).toBe('blåbærsyltetøy');
        expect(phpUnserialize('s:10:"$\u00a2\u20ac\ud841\udf0e";')).toBe(
            '$¢€𠜎'
        );
        expect(phpUnserialize('s:10:"$¢€𠜎";')).toBe('$¢€𠜎');
    });

    it('can parse a non-UTF-8 string', () => {
        expect(phpUnserialize('s:28:"�f���V�����b��r�[�t�n���~�q";')).toBe(
            '�f���V�����b��r�[�t�n���~�q'
        );
        expect(phpUnserialize('s:30:"ãƒ™ã‚¸ã‚¿ãƒªã‚¢ãƒ³ã§ã™ã‹ï¼Ÿ";')).toBe(
            'ãƒ™ã‚¸ã‚¿ãƒªã‚¢ãƒ³ã§ã™ã‹ï¼Ÿ'
        );
    });
    ...
    it('can parse an array', () => {
        expect(
            phpUnserialize('a:2:{i:0;s:5:"hello";i:1;s:5:"world";}')
        ).toEqual(['hello', 'world']);
    });

    it('can parse a dictionary', () => {
        expect(
            phpUnserialize('a:2:{s:5:"hello";i:0;s:5:"world";i:1;}')
        ).toEqual({ hello: 0, world: 1 });
        expect(
            phpUnserialize(
                'a:2:{s:6:"0b5f7j";s:6:"value1";s:10:"anotherKey";s:6:"value2";}'
            )
    ...
    it('can parse a reference', () => {
        expect(
            phpUnserialize('a:2:{s:5:"hello";i:42;s:5:"world";R:2;}')
...
describe('Object values', () => {
    ...
    it('can parse public members', () => {
        expect(
            phpUnserialize('O:3:"Foo":2:{s:5:"hello";i:0;s:5:"world";i:1;};')
        ).toEqual({ hello: 0, world: 1 });
    });
    ...
    it('can parse a circular reference', () => {
        ...
        expect(phpUnserialize('O:3:"Bar":1:{s:4:"self";r:1;}')).toEqual(
            expected
        );
    });

    it('can parse a numeric key', () => {
        expect(phpUnserialize('O:3:"key":2:{i:0;N;i:1;s:4:"main";}')).toEqual({
            0: null,
            1: 'main',
        });
    });
...
describe('Custom serializers', () => {
    it('can parse a SplDoublyLinkedList', () => {
        ...
        expect(phpUnserialize('C:19:"SplDoublyLinkedList":4:{i:0;}')).toEqual(
            expected
        );
    });
    ...
    it('can parse a SplObjectStorage', () => {
        const expected: any = {
            ...
            serialized:
                'x:i:2;O:8:"stdClass":1:{s:1:"a";O:8:"stdClass":0:{}},i:1;;r:4;,i:2;;m:a:0:{}',
        };
        expect(
            phpUnserialize(
    ...
    it('can parse a SplObjectStorage with subsequent value', () => {
        ...
        expect(
            phpUnserialize(
                'a:2:{i:0;C:16:"SplObjectStorage":33:{a:1:{s:2:"id";s:10:"caO8WPx0GQ";}}i:1;b:1;}'
            )
        ).toEqual(expected);
    });
});
...
describe('Invalid input', () => {
    it('throws exception on unknown type', () => {
        ...
        try {
            ...
            expect(e).toBeInstanceOf(UsageException);
            expect(e).toHaveProperty(
                'message',
                'Unknown type "" at position 0'
            );
        }
    });
    ...
    it('throws exception on unknown array key type', () => {
        ...
        try {
            ...
            expect(e).toHaveProperty(
                'message',
                'Unknown key type "d" at position 5'
            );
        }
    });
    ...
    it('throws exception on unknown object key type', () => {
        ...
        try {
            ...
            expect(e).toHaveProperty(
                'message',
                'Unknown key type "d" at position 14'
            );
        }
    });
    ...
    it('throws exception on malformed property name', () => {
        ...
        try {
            ...
            expect(e).toHaveProperty(
                'message',
                'Expected two <NUL> characters in non-public property ' +
