...
function readKey(): string|number {
    ...
    throw new UsageException(`Unknown key type "${type}" at position ${idx - 2}`);
}
...
function fixPropertyName(parsedName: string, baseClassName: string) {
    ...
    if (isString(parsedName) && "\u0000" === parsedName.charAt(0)) {
        ...
        throw new UsageException(`Expected two <NUL> characters in non-public property "${parsedName}" at position ${idx - parsedName.length - 2}`);
    }
    // public "property"
    return parsedName;
}
 ...
 */
export function phpUnserialize(input: string) {
    str = input;
    refStack = [];
    idx = 0;
    ridx = 0;
