...
import { ensureArray, isObject, isPromiseLike, isType, isUndefined, Constructor, GenericCallback, } from '@banquette/utils-type';
 ...
 */
export class ObservablePromise<CompleteT = any>
    implements ObservablePromiseInterface<CompleteT>
{
    ...
    public constructor(
        executor: ExecutorFunction<CompleteT>,
        private parent?: ObservablePromise
    ) {
        try {
            executor(
    ...
    public then<ResultT = CompleteT, RejectT = never, ProgressT = never>(
        ...
        return new ObservablePromise<ResultT>(
            (
                resolve: ResolveCallback<ResultT>,
     ...
     */
    public catchNotOf<RejectT = never>(
        type: Constructor | Array<Constructor>,
        onReject: onRejectCallback<RejectT>
    ): ObservablePromiseInterface<CompleteT | RejectT> {
     ...
     */
    public finally<ResultT = CompleteT, RejectT = never>(
        onSettle: onFinallyCallback<ResultT> | null
    ): ObservablePromiseInterface<ResultT | RejectT> {
        return new ObservablePromise<ResultT>(
            (resolve: ResolveCallback<ResultT>, reject: RejectCallback) => {
                let rejected = false;
                let result: any;
                return this.then(
                    (value: CompleteT) => {
     ...
     */
    private resolve(
        result: CompleteT | ObservablePromiseInterface<CompleteT>
    ): void {
    ...
    >(handlers: {
        ...
        onReject?: onRejectCallback<RejectT>;
        onProgress?: ProgressObserverInterface<ProgressT>;
    }): void {
        this.observers.push({
        ...
        if (this.status !== PromiseStatus.Pending) {
            this.dispatch(
                ObservablePromise.EventTypeFromStatus(this.status),
                this.result,
    ...
    private dispatch(
        ...
        if (isUndefined(observerIndex)) {
            ...
            throw new UsageException(
                `Out of bounds observer index (${observerIndex}), ${this.observers.length} observers in array.`
            );
        }
    }
     ...
     */
    private catchBasedOnType<RejectT>(
        types: Array<Constructor>,
        shouldMatch: boolean,
        onReject: onRejectCallback<RejectT>
    ): ObservablePromiseInterface<CompleteT | RejectT> {
        ...
        return new ObservablePromise<T[]>(
            (resolve: ResolveCallback<T[]>, reject: RejectCallback) => {
                ...
                for (let i = 0; i < collection.length; ++i) {
                    ObservablePromise.Resolve(collection[i])
                        .then(
                            ((_i: number) => {
                                return (result: T) => {
                                    results[_i] = result;
        ...
        return new ObservablePromise<T>(
            (resolve: ResolveCallback<T>, reject: RejectCallback) => {
                ...
                for (const item of collection) {
                    ObservablePromise.Resolve(item)
                        .then((result: T) => {
                            if (!settled) {
                                resolve(result);
                            }
     ...
     */
    public static ResolveAfterDelay<T = any>(
        delay: number,
        value?: T
    ): ObservablePromise<T> {
        return new ObservablePromise<T>((resolve: ResolveCallback<T>) => {
    ...
    private static EventTypeFromStatus(
        ...
        switch (status) {
            case PromiseStatus.Fulfilled:
                return PromiseEventType.resolve;
            case PromiseStatus.Rejected:
                return PromiseEventType.reject;
            default:
                return PromiseEventType.progress;
        }
    }
