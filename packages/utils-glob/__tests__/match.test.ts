...
function buildCandidatesIndex(input: any, parent: string = '/'): any {
    ...
    if (isObject(input)) {
        for (const current of Object.keys(input)) {
            const currentPath =
                !isNumeric(current) || isArray(input[current])
                    ? parent + (parent !== '/' ? '/' : '') + current
                    : parent;
            if (isString(input[current])) {
                const parts = input[current].split(':');
                output[parent + (parent !== '/' ? '/' : '') + parts[0]] =
                    parts.slice(1);
            } else {
                Object.assign(
                    output,
                    buildCandidatesIndex(input[current], currentPath)
                );
            }
        }
    }
...
describe('pattern matching', () => {
    ...
    const patternsTests: any = {
        ...
        globstar: {
            '/**/name': [
                ['/category/name', 2],
                ['/articles/0/category/name', 2],
                ['/articles/1/category/name', 2],
                ['/tags/0/name', 2],
                ['/tags/1/name', 2],
                1,
            ],
    ...
    const matchAndCheck = (pattern: string, candidate: any) => {
        const result: MatchResult = match(
            pattern,
            candidate[0],
            candidatesIndex[candidate[0]]
        );
        expect(
            Object.assign(result, { candidate: candidate[0] })
        ).toMatchObject({
                ...
                return () => {
                    ...
                    for (const pattern of patterns) {
                        test(pattern, () => {
                            ...
                            for (const candidate of patternsTests[_g][
                                pattern
                            ]) {
                                if (isNumber(candidate)) {
                                    defaultPatternMatch = candidate;
                                } else if (isNumeric(candidate[0])) {
                                    defaultPatternMatch = !isUndefined(
                                        candidate[0]
                                    )
                                        ? candidate[0]
                                        : defaultPatternMatch;
                                    defaultTagMatch = !isUndefined(candidate[1])
                                        ? candidate[1]
                                        : defaultTagMatch;
                                } else {
                            ...
                            expect.assertions(
                                Object.keys(candidatesIndex).length
                            );
                        });
                    }
                };
