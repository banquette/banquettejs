...
import { EventArg, EventDispatcherService, DispatchResult, DispatchResultStatus, } from '../src';
...
test('propagation tags', async () => {
    ...
    await expectFromDispatchAsync(tracker, event1, [
        'encoder1',
        'other',
        'encoder2',
    ]);
...
test('priority', () => {
    ...
    testSubscribeAndDispatchSync(event1, { c: [1], a: [0], b: [1] }, [
        'c',
        'b',
        'a',
    ]);
...
test('priority (async)', () => {
    ...
    testSubscribeAndDispatch(event1, { c: [1], a: [0], b: [1] }, [
        'c',
        'b',
        'a',
    ]);
...
test('unsubscribe', () => {
    ...
    const unsubscribeCallback = eventDispatcher.subscribe(
        event1,
        (event: EventArg) => {
            counter = 1000;
            event.stopPropagation();
    ...
    for (const key of Object.keys(subscribers)) {
        eventDispatcher.subscribe(
            ...
            ((
                ...
                sp: boolean,
                delay: number,
                exception: string | Exception | null
            ) => {
                ...
                key,
                subscribers[key][3] || false,
                subscribers[key][4] || 0,
                subscribers[key][5] || null
            ),
            subscribers[key][0],
            !isUndefined(subscribers[key][1])
                ? (subscribers[key][1] as any)
                : [],
            subscribers[key][2] || []
        );
    }
 ...
 */
function expectFromDispatch(
    type: symbol,
    expected: string[],
    sequential: boolean = false,
    tags: symbol[] = []
): void {
    const result: DispatchResult = eventDispatcher.dispatch<string>(
        type,
        null,
        sequential,
        tags
 ...
 * Dispatch an event and check if the results of a tracker are what is in "expected".
 */
async function expectFromDispatchAsync(
    tracker: SubscribersTracker,
    type: symbol,
    expected: string[],
    sequential: boolean = false,
    tags: symbol[] = []
 ...
 */
async function testSubscribeAndDispatch(
    type: symbol,
    subscribers: Subscribers,
    expected: string[]
): Promise<void> {
 ...
 * Call subscribeAndTrack and expectFromDispatch successively.
 */
function testSubscribeAndDispatchSync(
    type: symbol,
    subscribers: Subscribers,
    expected: string[]
): void {
