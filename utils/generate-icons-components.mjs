        ...
        listFiles: () => {
            ...
            const scanDir = (_path, nameParts = []) => {
                fs.readdirSync(_path).forEach(function(item) {
                    let fullPath = path.join(_path, item);
                    ...
                    if (typeof(output[name]) === 'undefined') {
                        output[name] = {fullName: nameParts.join('/'), versions: {}};
                    }
                    ...
                    output[name].versions[category] = fullPath;
                });
            };
            scanDir(baseDir);
            return output;
        ...
        listFiles: () => {
            ...
            fs.readdirSync(baseDir).forEach(function(type) {
                ...
                fs.readdirSync(typePath).forEach(function (icon) {
                    ...
                    if (typeof(output[name]) === 'undefined') {
                        output[name] = {fullName: type + '/' + name, versions: {}};
                    }
                    icon = icon.replace(/_/g, '-');
                    output[name].versions[version] = path.join(typePath, icon);
                });
            });
            return output;
        },
...
for (let libIndex = 0; libIndex < configurations.length; ++libIndex) {
    ...
    if (fs.existsSync(configuration.outputDir)) {
        fs.rmSync(configuration.outputDir, {recursive: true});
    }
    ...
    for (const filename of Object.keys(files)) {
        ...
        const className = camelize(`I-${configuration.libName}-${componentName}`);
        const componentDir = path.dirname(path.join(configuration.outputDir, files[filename].fullName));
        ...
        const versions = Object.keys(files[filename].versions);
        if (!versions.length) {
        ...
        for (let i = 0, j; i < versions.length; i++) {
            const token = tokenizeSvg(files[filename].versions[versions[i]]);
...
export default /* @__PURE__ */ ((_) => {
    return r('${componentName}', {
        ...
        imports.push(`export { default as ${className} } from '.${componentDir.replace(configuration.outputDir, '')}/${componentName}.component.vue';`);
        ++count;
    }
