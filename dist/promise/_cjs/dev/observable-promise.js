...
var promiseEventType = require('./promise-event-type.js');
var promiseStatus = require('./promise-status.js');
...
var ObservablePromise = /** @class */ (function () {
    function ObservablePromise(executor, parent) {
         ...
         */
        this.status = promiseStatus.PromiseStatus.Pending;
        /**
    ...
    ObservablePromise.prototype.catchOf = function (type, onReject) {
        return this.catchBasedOnType(ensureArray.ensureArray(type), true, onReject);
    };
    /**
    ...
    ObservablePromise.prototype.catchNotOf = function (type, onReject) {
        return this.catchBasedOnType(ensureArray.ensureArray(type), false, onReject);
    };
    /**
    ...
    ObservablePromise.prototype.cancel = function () {
        this.reject(new cancel_exception.CancelException());
    };
    /**
    ...
    ObservablePromise.prototype.timeout = function (delay) {
        ...
        return new ObservablePromise(function (resolve, reject, progress) {
            setTimeout(function () {
                if (_this.isPending()) {
                    _this.reject(new timeout_exception.TimeoutException());
                }
            }, delay);
    ...
    ObservablePromise.prototype.resolve = function (result) {
        return this.settle(result, promiseStatus.PromiseStatus.Fulfilled);
    };
    /**
     * Reject the promise.
     */
    ObservablePromise.prototype.reject = function (reason) {
        return this.settle(reason, promiseStatus.PromiseStatus.Rejected);
    };
    /**
    ...
    ObservablePromise.prototype.notify = function (value) {
        ...
        this.dispatch(promiseEventType.PromiseEventType.progress, value);
    };
    /**
    ...
    ObservablePromise.prototype.subscribe = function (handlers) {
        ...
        if (this.status !== promiseStatus.PromiseStatus.Pending) {
        ...
        for (var _b = 0, _c = this.progressHistory; _b < _c.length; _b++) {
            var value = _c[_b];
            this.dispatch(promiseEventType.PromiseEventType.progress, value, this.observers.length - 1);
        }
    };
    /**
    ...
    ObservablePromise.prototype.settle = function (result, status) {
        var _this = this;
        setTimeout(function () {
            if (status === promiseStatus.PromiseStatus.Pending) {
                throw new usage_exception.UsageException('You can\'t set the pending status.');
            }
            if (_this.status !== promiseStatus.PromiseStatus.Pending) {
                return;
            }
            ...
            if (isType.isType(result, isPromiseLike.isPromiseLike)) {
    ...
    ObservablePromise.prototype.dispatch = function (type, value, observerIndex) {
        var doDispatch = function (observer) {
            switch (type) {
                case promiseEventType.PromiseEventType.resolve:
                    {
                        observer.onResolve(value);
                    }
                    break;
                case promiseEventType.PromiseEventType.reject:
                    {
                        observer.onReject(value);
                    }
                    break;
                case promiseEventType.PromiseEventType.progress:
                    {
        ...
        if (isUndefined.isUndefined(observerIndex)) {
            ...
            throw new usage_exception.UsageException("Out of bounds observer index (".concat(observerIndex, "), ").concat(this.observers.length, " observers in array."));
        }
    };
    /**
    ...
    ObservablePromise.prototype.catchBasedOnType = function (types, shouldMatch, onReject) {
        ...
        return this.then(function (value) { return value; }, function (reason) {
            var pos = isObject.isObject(reason) ? types.indexOf(reason.constructor) : null;
    ...
    ObservablePromise.All = function (collection) {
        return new ObservablePromise(function (resolve, reject) {
            collection = ensureArray.ensureArray(collection);
            var done = 0;
    ...
    ObservablePromise.MinDelay = function (delay, executor) {
        ...
        return new ObservablePromise(function (resolve, reject) {
            var forward = function (type, result) {
                if (type === promiseEventType.PromiseEventType.resolve) {
                    resolve(result);
                }
                else if (type === promiseEventType.PromiseEventType.reject) {
                    reject(result);
                }
            };
            ...
            sub.then(function (result) {
                forwardIfDelay(promiseEventType.PromiseEventType.resolve, result);
            }).catch(function (reason) {
                forwardIfDelay(promiseEventType.PromiseEventType.reject, reason);
            });
        });
    };
    /**
    ...
    ObservablePromise.EventTypeFromStatus = function (status) {
        switch (status) {
            case promiseStatus.PromiseStatus.Fulfilled: return promiseEventType.PromiseEventType.resolve;
            case promiseStatus.PromiseStatus.Rejected: return promiseEventType.PromiseEventType.reject;
            default: return promiseEventType.PromiseEventType.progress;
        }
    };
    /**
