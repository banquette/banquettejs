...
import { CallContext, FormEvents, BasicState, ValidationStrategy } from './constant.js';
import { BeforeValueChangeFormEvent } from './event/before-value-change.form-event.js';
...
var FormControl = /** @class */ (function (_super) {
    ...
    function FormControl(value, validator) {
        ...
        _this.setValue = (function () {
            ...
            return function (value) {
                try {
                    if (locked || areEqual(_this.lastValue, value)) {
                        return;
                    }
                    locked = true;
                    var beforeValueChangeEvent_1 = new BeforeValueChangeFormEvent(_this, _this.lastValue, value);
                    _this.dispatch(FormEvents.BeforeValueChange, beforeValueChangeEvent_1);
                    ...
                    if (!areEqual(beforeValueChangeEvent_1.newValue, value)) {
                        _this.viewModels.forEach(function (vm) {
                    ...
                    _this.value = value;
                    _this.dispatch(FormEvents.ValueChanged, function () { return new ValueChangedFormEvent(_this, _this.lastValue, _this.value); });
                    _this.lastValue = cloneDeepPrimitive(_this.value);
                    _this.viewModels.forEach(function (vm) {
                        if (vm !== _this.focusedViewModel) {
                            vm.setValue(value);
                        }
                    });
                    _this.markBasicState(BasicState.Dirty);
                    if (!areEqual(_this.defaultValue, value)) {
                        _this.markBasicState(BasicState.Changed);
                    }
                    else {
                        _this.unmarkBasicState(BasicState.Changed);
                    }
                    if (!_this.hasContext(CallContext.Reset)) {
                        _this.validateIfStrategyMatches(ValidationStrategy.OnChange);
                    }
                    if (_this.parent !== null && !_this.hasContext(CallContext.Parent)) {
                        _this.updateValue();
                    }
                }
                finally {
        ...
        _this.value = value;
        _this.lastValue = cloneDeepPrimitive(value);
        _this.defaultValue = cloneDeepPrimitive(value);
    ...
    FormControl.prototype.onBeforeValueChange = function (callback, priority) {
        return this.subscribe(FormEvents.BeforeValueChange, callback, priority, true);
    };
    /**
    ...
    FormControl.prototype.buildContextualizedViewModelApi = function (map, viewModel) {
        var _this = this;
        var wrapped = {};
        for (var _i = 0, _a = getObjectKeys(map); _i < _a.length; _i++) {
            var key = _a[_i];
