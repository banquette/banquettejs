...
var insertInString = require('@banquette/utils-string/_cjs/dev/insert-in-string');
var extractCssSelectors = require('./extract-css-selectors.js');
...
function injectContextInCssSource(source, themeId, variantId, scopeId) {
    source = trim.trim(source.replace(/\/\*[\s\S]*?\*\/|\/\/.*/g, ''));
    var selectors = extractCssSelectors.extractCssSelectors(source).reverse();
    ...
    for (var i = 0; i < selectors.length; ++i) {
        ...
        if (scopeId !== null) {
                ...
                if (c === ':') {
                    ...
                    if (selectors[i][0].substring(j, j + 5) === ':deep') {
                        ...
                        while (end < selectors[i][0].length && selectors[i][0][end].match(/\s/) && ++end)
                            { }
                        source = trim.trim(source.substring(0, offset1) + source.substring(offset1 + (end - i)));
            ...
            source = insertInString.insertInString(source, scopeIdPos + selectors[i][1], "[".concat(scopeId, "]") + (scopeIdPos === 0 ? ' ' : ''));
        }
        // Variant id
        if (selectors[i][0][0] === '&') {
            ...
            while (source[offset].match(/\s/) && ++offset < source.length)
                { }
            source = insertInString.insertInString(source, offset, "[".concat(variantId, "]"));
        }
        else {
            source = insertInString.insertInString(source, selectors[i][1], "[".concat(variantId, "] "));
        }
        // Theme id
        source = insertInString.insertInString(source, selectors[i][1], ".".concat(themeId, " ").concat(globals.join(' ') + (globals.length ? ' ' : '')));
    }
    return source;
}

exports.injectContextInCssSource = injectContextInCssSource;
