...
var getThemesForComponent = require('./utils/get-themes-for-component.js');
var matchVariants = require('./utils/match-variants.js');
var splitVariantString = require('./utils/split-variant-string.js');
var vueThemes = require('./vue-themes.js');

var BindThemeDirective = /** @class */ (function (_super) {
    _tslib.__extends(BindThemeDirective, _super);
    ...
    BindThemeDirective.prototype.created = function (el, bindings) {
        var _this = this;
        this.instance = converters.anyToTsInst(bindings.instance);
        var metadata = converters.anyToComponentMetadata(bindings.instance);
        var configuration = metadata ? metadata.themeable : null;
        if (!this.instance || !metadata) {
            throw new usage_exception.UsageException('The "v-bt-bind-theme" directive can only be used on vue-typescript components.');
        }
        if (!configuration) {
            throw new usage_exception.UsageException('You must add a @Themeable() decorator to your component to use the "v-bt-bind-theme" directive.');
        }
        this.metadata = metadata;
        this.configuration = configuration;
        this.globalUnsubscribeFns.push(vueThemes.VueThemes.OnChanged(function () {
    ...
    BindThemeDirective.prototype.computeChanges = function (el) {
        var _this = this;
        var themes = getThemesForComponent.getThemesForComponent(this.instance);
        var themesParentsTrackers = [];
        var activeVariantsAttributes = [];
        var expectedVariants = splitVariantString.splitVariantString(this.instance[this.configuration.prop] || '');
        this.instance[constants.ACTIVE_VARIANTS] = [];
        ...
        for (var _i = 0, themes_1 = themes; _i < themes_1.length; _i++) {
            ...
            var _loop_1 = function (variantCandidate) {
                if (matchVariants.matchVariant(variantCandidate, expectedVariants, this_1.instance)) {
                    this_1.instance[constants.ACTIVE_VARIANTS].push(variantCandidate);
                    activeVariantsAttributes.push(variantCandidate.uid);
                    if (variantCandidate.applyIds.length > 0) {
                        this_1.instance[constants.ACTIVE_VARIANTS] = this_1.instance[constants.ACTIVE_VARIANTS].concat(variantsCandidates.filter(function (v) {
                            if (v.publicId !== null && variantCandidate.applyIds.indexOf(v.publicId) > -1 && _this.instance[constants.ACTIVE_VARIANTS].indexOf(v) < 0) {
        ...
        if (this.activeVariantsAttributesStr !== activeVariantsAttributesStr) {
            ...
            for (var _e = 0, _f = this.instance[constants.ACTIVE_VARIANTS]; _e < _f.length; _e++) {
                var item = _f[_e];
                el.setAttribute('data-' + item.uid, '');
                this.computeChangesUnsubscribeFns.push(item.onChange(proxy.proxy(this.scheduleForceUpdate, this)));
            }
    ...
    BindThemeDirective = _tslib.__decorate([
        directive_decorator.Directive('bt-bind-theme')
    ], BindThemeDirective);
    return BindThemeDirective;
}(vue.Vue));

exports.BindThemeDirective = BindThemeDirective;
