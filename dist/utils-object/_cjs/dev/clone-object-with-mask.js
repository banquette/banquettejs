...
function cloneObjectWithMask(source, mask) {
    var sourceIsArray = isArray.isArray(source);
    var clone = sourceIsArray ? [] : {};
    if (isObject.isObject(source)) {
        if (isObject.isObject(mask, true)) {
            ...
            if (maskKeys.length === 1 && maskKeys[0] === '*') {
                ...
                for (var _a = 0, _b = Object.keys(mask); _a < _b.length; _a++) {
                    ...
                    else if (mask !== false && !isUndefined.isUndefined(source[key])) {
            ...
            return cloneDeep.cloneDeep(source);
        }
    }
    else {
        return source;
