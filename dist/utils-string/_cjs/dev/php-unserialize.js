...
function readKey() {
    ...
    throw new usage_exception.UsageException("Unknown key type \"".concat(type, "\" at position ").concat(idx - 2));
}
...
function parseNext() {
    ...
    throw new usage_exception.UsageException("Unknown type \"".concat(type, "\" at position ").concat(idx - 2));
}
function fixPropertyName(parsedName, baseClassName) {
    var class_name;
    var prop_name;
    var pos;
    if (isString.isString(parsedName) && "\u0000" === parsedName.charAt(0)) {
        // "<NUL>*<NUL>property"
        ...
        throw new usage_exception.UsageException("Expected two <NUL> characters in non-public property \"".concat(parsedName, "\" at position ").concat(idx - parsedName.length - 2));
    }
    // public "property"
    return parsedName;
}
