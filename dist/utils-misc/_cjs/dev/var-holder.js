...
var VarHolder = /** @class */ (function () {
    function VarHolder(bag) {
        if (!isUndefined.isUndefined(bag)) {
    ...
    VarHolder.prototype.get = function (key, defaultValue) {
        ...
        for (var i = 0; i < keys.length; ++i) {
            if (isUndefined.isUndefined(bag[keys[i]]) || (!isObject.isObject(bag[keys[i]]) && i < keys.length - 1)) {
                return defaultValue;
            }
            bag = bag[keys[i]];
        }
    ...
    VarHolder.prototype.set = function (key, value) {
        ...
        for (var i = 0; i < keys.length - 1; ++i) {
            if (isUndefined.isUndefined(bag[keys[i]])) {
                bag[keys[i]] = {};
            }
            if (!isObject.isObject(bag[keys[i]])) {
                throw new usage_exception.UsageException('The key "' + keys[i] + '" is already used by a non object value.');
            }
            bag = bag[keys[i]];
        }
    ...
    VarHolder.prototype.add = function (values) {
        for (var _i = 0, _a = getObjectKeys.getObjectKeys(values); _i < _a.length; _i++) {
            var key = _a[_i];
