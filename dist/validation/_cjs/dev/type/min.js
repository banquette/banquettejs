 ...
 */
function Min(count, options) {
    if (options === void 0) { options = {}; }
    var finalOptions = utils.assignOptionsDefaults(options, 'auto', 'min');
    var treatAs = !isString.isString(options) ? (options.treatAs || 'auto') : 'auto';
    return createValidator.createValidator({
        validate: function (context) {
            var valid = true;
            var defaultMessage = 'Must be greater or equal to %count%.';
            if ((isString.isString(context.value) && treatAs === 'auto') || treatAs === 'string') {
                ...
                defaultMessage = "Must be at least %count% character".concat((count > 1 ? 's' : ''), " long.");
            }
                ...
                defaultMessage = "Must contain at least %count% item".concat((count > 1 ? 's' : ''), ".");
            }
            if (!valid) {
